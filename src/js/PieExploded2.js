...
export class PieExploded {
	constructor(container, data, labels_pie, opts) {
		...
		let asset_class_comment =
			typeof labels_pie.asset_class === 'undefined' ? 'Asset Class' : labels_pie.asset_class;
		let comment =
			typeof labels_pie.comment === 'undefined' ? ['', ''] : Array.from(labels_pie.comment); // create a new variable so we don't modify the original labels
		comment[0] = comment[0].replace('_PORT_', asset_class_comment);
		let numbers_long =
			typeof labels_pie.comment === 'undefined'
				? { M: ' million', G: ' billion', T: ' trillion' }
				: labels_pie.numbers_long;
		...
		this.text_left_offset = Math.min(
			100,
			this.margin.left + this.exploded_offset - tech_label_width
		);
	...
	update(data, labels_plot, comment_position) {
		...
		var slices = chart.svg
			...
			.append('g')
			.attr('transform', (d) =>
				d.data.exploded ? 'translate(' + chart.exploded_offset + ' 0)' : 'translate(0 0)'
			);
		slices
			.append('path')
		...
		var linesLabels = slicelabels
			...
			.attr('stroke-width', 1)
			.attr('points', (d) => [
				point_coord(d.midAngle, chart.radius),
				[d.elbowx, d.elbowy],
		...
		function relaxLabels() {
			...
			textLabels.each(function (d, i) {
				...
				textLabels.each(function (d, i) {
					...
					deltaY = y1 - y2;
					if (Math.abs(deltaY) > chart.minLabelSpacing) return;
		...
		let comment_height = Math.max(
			(4 * chart.height) / 5,
			d3.max(data_ready, (d) => (d.textx > 0 ? d.texty : 0)) + chart.height / 2 + 30
		);
