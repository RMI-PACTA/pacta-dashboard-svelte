...
export class company_bubble {
	constructor(container, data, labels, opts) {
		...
		this.xtitle =
			typeof labels.xtitle === 'undefined'
				? 'Share of the Client capacity in low-carbon technologies'
				: labels.xtitle;
		...
		let size = Math.min(
			this.chrtwidth - this.margin.left - this.margin.right,
			this.chrtheight - this.margin.top - this.margin.bottom
		);
		this.width = size;
		this.height = size;
		...
		this.svg
			...
			.attr(
				'transform',
				'translate(' + this.width / 2 + ' ,' + (this.height + this.margin.top - 35) + ')'
			)
			.style('text-anchor', 'middle');
		this.svg
			.append('text')
		...
		function update() {
			let subdata;
			subdata = chart.data.filter(
				(d) =>
					(d[chart.filter] == sector_selector.value) &
					(d.asset_class_translation == class_selector.value)
			);
			...
			if (chart.xymaxfixed) {
				...
				chart.y
					.domain([0 - buffer, (chart.ymaxfixed ? 1 : d3.max([1, d3.min([2, ymax])])) + buffer])
					.nice();
			}
			...
			chart.container
				.select('.subtitle')
				.text(chart.subtitlesuffix != '' ? chart.current_selection + chart.subtitlesuffix : '');
