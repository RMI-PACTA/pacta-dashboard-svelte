...
export class choropleth {
	constructor(container, data, labels, opts) {
		...
		this.color_range =
			typeof opts.color_range === 'undefined' ? ['#e8ebf1', '#1b324f'] : opts.color_range;
		...
		const title_what =
				typeof labels.title_what === 'undefined' ? 'Regional exposure of ' : labels.title_what,
		...
		function appendOptionsToTechSelector(tech_selector, data, selected_option) {
			...
			tech_selector.options[
				Math.max([...tech_selector.options].map((d) => d.text).indexOf(selected_option), 0)
			].selected = 'selected';
		...
		this.topo = feature(world, world.objects.countries);
		...
		function mouseover(d) {
			...
			chart.tooltip
				.html(d.properties['name'] + '<br>' + tip_format(value) + ' ' + unit)
		...
		function legend(g) {
			...
			const slctd_option = chart.data
				.filter((d) => d.option == tech_selector.value)
				.map((d) => d.option_translation)[0];

			const unit_text = chart.data
				.filter((d) => d.option == tech_selector.value)
				.map((d) => d.unit_translation)[0];
		...
		function map_update_option() {
			let subset = chart.data.filter(
				(d) =>
					(d.option == tech_selector.value) & (d.asset_class_translation == class_selector.value)
			);
