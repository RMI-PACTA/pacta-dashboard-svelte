...
export class techexposure {
	constructor(container, data, labels, opts) {
		...
		const title_what =
				typeof labels.port_label === 'undefined' ? ': Technology mix ' : labels.title_what,
			title_opts_how =
				typeof labels.port_label === 'undefined'
					? { opt_asset: 'as % of assets under management', opt_sec: 'as % of sector' }
					: labels.title_opts_how,
			title_how = typeof labels.port_label === 'undefined' ? 'compared to ' : labels.title_how,
			caption_market =
				typeof labels.port_label === 'undefined' ? 'Equity market: ' : labels.caption_market,
		...
		const tooltip = d3
			...
			.style('display', 'none');
		let svg = d3.select(chart_div).append('svg').attr('width', ttl_width);
				...
				legend_order.forEach(function (item) {
					if (
						...
						let idx = legend_data_unordered.findIndex(
							(d) => d.ald_sector == item.ald_sector && d.technology == item.technology
						);
		...
		function change_class() {
			...
			let selected_market =
				typeof market_selector.value === 'undefined' ? 'Global' : market_selector.value;
			...
			let benchmark_names = d3
				.map(
					subdata.filter((d) => !d.this_portfolio),
					(d) => d.portfolio_name
				)
				.keys()
				.sort();
		...
		function update() {
			...
			let port_sectors = d3
				.map(
					subdata.filter((d) => d.this_portfolio == true),
			...
			let port_labels = port_labels_grp.selectAll('.portfolio_label').data(
				port_sectors
					.map((d) => {
						return [
			...
			let legend_data_unordered = subdata
