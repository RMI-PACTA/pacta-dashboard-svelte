...
export class techexposure_company {
	constructor(container, data_down, labels, opts, data_up) {
		...
		d3.select(container_div).attr('chart_type', 'techexposure_company');
		...
		const default_color =
			typeof opts.default_color === 'undefined' ? '#00c082' : opts.default_color; // color - Green from 2DII palette

		// set labels
		labels = typeof labels === 'undefined' ? {} : labels;
		const title_what =
				typeof labels.title_what === 'undefined'
					? ': Future technology mix for the largest holdings (by portfolio weight)'
					: labels.title_what,
			title_how =
				typeof labels.title_how === 'undefined' ? 'as % of sector for ' : labels.title_how,
			...
			footnote_lab =
				typeof labels.footnote === 'undefined'
					? { befor_scen: '* Aligned to ', after_scen: ' scenario in year ', after_year: '.' }
					: labels.footnote;
    ...
    */
		...
		let class_names = d3
			.map(data_down, (d) => d.asset_class_translation)
			.keys()
			.sort();
		...
		let data_down_filtered = data_down.filter(
			(d) => d.asset_class_translation == class_selector.value
		);

		// sector selector
		let sector_names = d3
			.map(data_down_filtered, (d) => d.ald_sector_translation)
			.keys()
			.sort();
  ...
  */
    ...
    */
		...
		function drawStackedBarGroup(
			...
			y_labels_group
				.attr('transform', 'translate( ' + 0 + ', ' + graph_start_height + ' )')
		...
		function update() {
			...
			yWeights_group
				.attr(
					'transform',
					'translate( ' + width + ', ' + (groups_up.length * bar_height_space + port_gap) + ' )'
				)
				.call(
					d3
						.axisRight(yWeights)
			...
			test_svg
				...
				.text(long_label_leg)
				.each(function () {
					label_width_leg = this.getBBox().width;
				});
			test_svg.remove();
			...
			footnote.attr(
				...
				'translate(' +
					width +
					',' +
					(height + margin.top + (Math.floor(legend_data.length / nr_labels_in_row) + 1) * 30) +
					')'
			);
