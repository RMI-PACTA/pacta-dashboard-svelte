...
export class techexposure_future {
	constructor(container, data, labels, opts) {
		...
		const default_market =
			typeof opts.default_market === 'undefined' ? 'Global Market' : opts.default_market;
		const default_scenario =
			typeof opts.default_scenario === 'undefined' ? 'ETP2017_B2DS' : opts.default_scenario;
		var legend_order = opts.legend_order;

		// set labels
		labels = typeof labels === 'undefined' ? {} : labels;
		const title_up =
				typeof labels.title_up === 'undefined'
					? ': Future technology mix as % of sector based on '
					: labels.title_up,
			title_scenario =
				typeof labels.title_scenario === 'undefined' ? ' scenario' : labels.title_scenario,
			title_down = typeof labels.title_down === 'undefined' ? 'compared to ' : labels.title_down,
			title_market =
				typeof labels.title_market === 'undefined' ? ' as a subset of ' : labels.title_market,
			hover_over_sec =
				typeof labels.hover_over_sec === 'undefined'
					? { before_sec: ' of ', after_sec: ' sector' }
	    ...
	    */
		...
		function orderLegendDataIfPossible(legend_data_unordered, legend_order) {
			if (typeof legend_order === 'undefined') {
				...
				legend_order.forEach(function (item) {
					if (
						chart_sectors.includes(item.ald_sector) &&
						chart_technologies.includes(item.technology)
					) {
						let idx = legend_data_unordered.findIndex(
							(d) => d.sector == item.ald_sector && d.technology == item.technology
						);
				...
				if (legend_data_unordered.length > legend_data.length) {
					console.warn(
						'Not all sector/technology pairs from the data were found in sector_order variable. The legend shown is incomplete. Please, amend the sector_order.csv file. '
					);
				}
		...
		// scenario selector
		let scenario_names = d3
			.map(data_filtered, (d) => d.scenario)
			.keys()
			.sort();
		let scenario_selector = document.createElement('select');
		scenario_selector.classList = 'techexposure_scenario_selector inline_text_dropdown';
		scenario_selector.addEventListener('change', change_class);
		scenario_names.forEach((scenario_name) =>
			scenario_selector.add(new Option(scenario_name, scenario_name))
		);
		scenario_selector.options[Math.max(scenario_names.indexOf(default_scenario), 0)].selected =
			'selected';
		...
		// market selector
		let market_names = d3
			.map(data_filtered, (d) => d.equity_market_translation)
			.keys()
			.sort();
		...
		market_names.forEach((market_name) =>
			market_selector.add(new Option(market_name, market_name))
		);
		market_selector.options[Math.max(market_names.indexOf(default_market), 0)].selected =
			'selected';
		...
		function getTechnologyDataForStacking(data, un_value_types) {
			...
			un_value_types.forEach((item, index) => {
				subdata_tech[index] = {};
				subdata_tech[index]['val_type'] = item;
				data
					.filter((d) => d.val_type == item)
					.forEach((x) => {
						subdata_tech[index]['val_type_translation'] = x.val_type_translation;
		...
		function getSubsetData(data, asset_class, scenario, benchmark, market) {
			...
			subdata = subdata.filter(
				(d) => (d.this_portfolio == true) | (d.portfolio_name_translation == benchmark)
			);
	    ...
	    */
		...
		function drawStackedBarGroupOnEmptyChart(
			bars_group,
			x,
			y,
			y_labels_group,
			...
			y_labels_group
				.append('g')
				.attr('transform', 'translate( ' + -20 + ', ' + graph_start_height + ' )')
			...
			if (stackedData.length != 0) {
				...
				throw new Error('Error during stacking the data: empty array was returned.');
			}
			...
			bars_group
				...
				.attr('y', function (d) {
					return graph_start_height + y(d.data.val_type_translation);
				})
		...
		function drawBarGroupPerSector(
			...
			let groups_plan = d3
				.map(subdata_plan, function (d) {
					return d.val_type_translation;
				})
				.keys();
			let groups_scen = d3
				.map(subdata_scen, function (d) {
					return d.val_type_translation;
				})
				.keys();

			drawStackedBarGroupOnEmptyChart(
				bars_group,
				x,
				y,
				y_labels_group,
			...
			drawStackedBarGroupOnEmptyChart(
				...
				subgroups,
				groups_scen,
				graph_start_height + groups_plan.length * bar_height + scen_gap,
				graph_width,
				bar_height,
				bar_gap,
				sector
			...
			subgroups.forEach(function (index, item) {
				var technology_translation = subdata.filter((d) => d.technology == item)[0][
				...
				let data_to_append = {
					...
					technology: item,
					class: sector + ' ' + item,
					sector_translation: sector_translation,
					technology_translation: technology_translation
				};
		...
		function change_class() {
			...
			scenario_selector.options[Math.max(scenario_names.indexOf(selected_scenario), 0)].selected =
				'selected';

			let benchmark_names = d3
				.map(
					data_filtered.filter((d) => !d.this_portfolio),
					(d) => d.portfolio_name_translation
				)
				.keys()
				.sort();
			benchmark_selector.length = 0;
			benchmark_names.forEach((benchmark_name) =>
				benchmark_selector.add(new Option(benchmark_name, benchmark_name))
			);
			benchmark_selector.options[
				Math.max(benchmark_names.indexOf(selected_benchmark), 0)
			].selected = 'selected';
		...
		function update() {
			...
			var bar_height_space =
				(height - (unique_sectors.length - 1) * sect_gap - (unique_sectors.length - 1) * scen_gap) /
				(nr_bars_sec * unique_sectors.length);
			...
			for (var i = 0; i < unique_sectors.length; i++) {
				legend_data_unordered = drawBarGroupPerSector(
					bars_group,
					x,
					y,
					y_labels_group,
					subdata,
					unique_sectors[i],
					i * (bar_height_space * nr_bars_sec + scen_gap + sect_gap),
					width,
					bar_height_space,
					bar_gap,
					scen_gap,
					legend_data_unordered
				);
			...
			legend_data.forEach(function (index, item) {
			...
			for (var i = 1; i < unique_sectors.length; i++) {
				tech_in_prev_sectors[i] =
					...
					legend_data.filter((obj) => obj.sector === unique_sectors[i - 1]).length;
			}
