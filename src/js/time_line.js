...
export class time_line {
	constructor(container, data, labels, opts) {
		...
		let scen_line_color =
			typeof opts.scen_line_color === 'undefined' ? '#00c082' : opts.scen_line_color;
		...
		let default_sector =
			typeof opts.default_sector === 'undefined' ? 'Aviation' : opts.default_sector;
		...
		const title_what =
				typeof labels.title_what === 'undefined'
					? ' : 5-year emission intensity trend of '
					: labels.title_what,
			caption_alloc =
			...
			hoverover_value =
				typeof labels.hoverover_value === 'undefined' ? 'Value: ' : labels.hoverover_value,
			footnote =
		...
		// sector selector
		let sector_names = d3
			.map(data, (d) => d.sector_translation)
			.keys()
			.sort();
		...
		let disabled = sector_names.map(
			(sector) => data.filter((d) => d.sector_translation == sector).map((d) => d.disabled)[0]
		);
		...
		function change_class() {
			...
			let sector_names = d3
				.map(subdata, (d) => d.sector_translation)
				.keys()
				.sort();
			sector_names.forEach((sector_name) =>
				sector_selector.add(new Option(sector_name, sector_name))
		...
		function update() {
			...
			//axes
			const num_of_years =
				1 + Math.abs(chart.x.domain().reduce((a, b) => a.getFullYear() - b.getFullYear()));
			let tick_labels = d3
				.map(subset, (d) => d.year)
				.keys()
			...
			chart.svg
				.select('.yaxis')
				.transition()
				.call(d3.axisLeft(chart.y).ticks(6).tickFormat(d3.format('.3s')));
			...
			let legend = legend_group.selectAll(null).data(legend_data);
