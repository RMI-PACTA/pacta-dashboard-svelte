	...
	constructor(container, data, labels, opts) {
		...
		const title_what =
				typeof labels.title_what === 'undefined'
					? ': Production trajectory of '
					: labels.title_what,
			title_how = typeof labels.title_how === 'undefined' ? 'compared to ' : labels.title_how,
			caption_alloc =
				typeof labels.caption_alloc === 'undefined' ? 'Allocation method: ' : labels.caption_alloc,
			caption_market =
				typeof labels.caption_market === 'undefined' ? 'Equity market: ' : labels.caption_market,
			caption_geography =
				typeof labels.caption_geography === 'undefined'
					? 'Scenario geography: '
					: labels.caption_geography,
			ytitle = typeof labels.ytitle === 'undefined' ? ' production in ' : labels.ytitle,
			portfolio_label =
				typeof labels.portfolio_label === 'undefined' ? 'Portfolio' : labels.portfolio_label,
			benchmark_label =
				typeof labels.benchmark_label === 'undefined' ? 'Benchmark' : labels.benchmark_label,
			caption_source =
				typeof labels.caption_source === 'undefined' ? 'Scenario source: ' : labels.caption_source,
			label_dots_legend =
				typeof labels.label_dots_legend === 'undefined'
					? { top: 'Yearly production', bottom: 'volume' }
					: labels.label_dots_legend,
			footnote =
				typeof labels.footnote === 'undefined' ? '* start date of the analysis' : labels.footnote,
			hoverover_value_label =
				typeof labels.hoverover_value_label === 'undefined'
					? '(Planned) yearly production: '
					: labels.hoverover_value_label;
		...
		function appendOptionsToTechSelector(tech_selector, data, selected_option) {
			...
			let sector_tech_grps = rollups(
				data_not_benchmark,
				(v) =>
					d3
						.map(v, (d) => d.technology_translation)
						.keys()
						.sort(),
				(d) => d.ald_sector_translation
			);
			...
			tech_selector.options[
				Math.max(
					sector_tech_grps
						.map((d) => d[1])
						.flat()
						.indexOf(selected_option),
					0
				)
			].selected = 'selected';
		...
		function reset_benchmark(data) {
			...
			benchmark_selector.options[
				Math.max(0, benchmark_names.indexOf(selected_benchmark))
			].selected = 'selected';
		...
		function reset_allocation(data) {
			...
			allocation_names.forEach((allocation_name) =>
				allocation_selector.add(new Option(allocation_name, allocation_name))
			);
		...
		function reset_source(data) {
			...
			source_names.forEach((source_name) =>
				source_selector.add(new Option(source_name, source_name))
			);
			source_selector.options[Math.max(0, source_names.indexOf(selected_source))].selected =
				'selected';
		...
		function update() {
			...
			let selected_class = class_selector.value;
			let selected_tech = tech_selector.value;
			let selected_tech_org = data.filter((d) => d.technology_translation == selected_tech)[0][
				'technology'
			];
			let selected_benchmark = benchmark_selector.value;
			let selected_allocation = allocation_selector.value;
			...
			if (subdata.length == 0) {
				DisplayNoDataMessage(
					'No data available for selected combination of filters. Try changing the allocation method, equity market, scenario geography or scenario source.'
				);
			...
			let benchmark_data = subdata.filter(
				(d) => d.portfolio_name_translation == selected_benchmark
			);
			if (benchmark_data.length == 0) {
				console.log(
					'No benchmark data available for selected combination of benchmark, allocation method, equity market, scenario geography and scenario source. Try changing the selection.'
				);
			}
